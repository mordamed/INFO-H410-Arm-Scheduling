...
\definecolor{bayesColor}{HTML}{1F77B4}     % blue   — Bayesian
\definecolor{cspColor}{HTML}{FF7F0E}       % orange — CSP
\definecolor{dqnColor}{HTML}{2CA02C}       % green  — MDP/DQN
\definecolor{techGray}{HTML}{555555}       % deep-dive accent
\definecolor{techBg}{HTML}{F2F2F2}         % deep-dive fill (very light)
...
\newtcolorbox{underhoodbox}[1][]{%
  ...
  fonttitle=\bfseries\small\color{techGray},
  title={#1},
...
\begin{underhoodbox}[Under the hood]
  CP-SAT is a \emph{lazy-clause-generation} solver: integer variables
  $t_i$ are kept in their finite-domain (FD) form, and clauses over the
  underlying order-encoding literals $[\![t_i \le v]\!]$ are generated
  on demand. Our reified disjunction (eq.~4) is internally decomposed
  into half-reified linear inequalities
  $b \Rightarrow (t_A - t_B \ge k)$ and $\neg b \Rightarrow (t_B - t_A \ge k)$,
  each propagated by a bounds-consistency engine using the two-watched-literal
  scheme inherited from MiniSAT. When two FD propagators derive
  contradictory bounds, the engine triggers \emph{Conflict-Driven Clause
    Learning} (CDCL): it walks the implication graph back to the First
  Unique Implication Point (1-UIP), records the cut as a learned no-good
  clause, and non-chronologically backjumps to the highest decision level
  still consistent with it~\cite{ortools}. Learned clauses prune symmetric
  branches elsewhere in the tree, which is why $n=50$ closes in $<1$~s
  despite having $O(|A|\!\cdot\!|B|)$ reification variables. The four
  parallel workers run a \emph{portfolio} of complementary strategies
  that share learned clauses through a lock-free pool, giving near-linear
  speed-up on disjoint subproblems.
\end{underhoodbox}
...
backtracking solver is used, limited to $n \le 15$. This implements plain chronological backtracking with
forward checking but no clause learning.
...
(3)~DQN provides near-instantaneous \emph{inference} after training, but is highly sensitive to reward shaping and training distribution design. Its ability to generalize depends on the diversity of the instruction blocks encountered during training.

Future work includes extracting direct CPT matrices from physicalCortex-M4 oscilloscopes, training DQN on higher security distances $k$ and stochastic training environments. Another promising direction is a  hybrid CSP\,+\,RL approach where the CSP provides a feasibility oracle for the agent.
...
\noindent Build the Docker image:
\begin{small}
\begin{verbatim}
...
\noindent Run the original benchmark:
\begin{small}
\begin{verbatim}
...
\noindent Run the generalist DQN training and evaluation:
\begin{small}
\begin{verbatim}
docker run --rm --gpus all \
  -v "${PWD}/experiments:/app/experiments" \
  arm-scheduler \
  python3 experiments/train_generalist.py
\end{verbatim}
\end{small}
