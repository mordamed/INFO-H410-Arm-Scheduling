...
A larger $k$ artificially increases the makespan by forcing longer gaps between shares. CSP and Bayesian solvers handle this gracefully without performance degradation, as constraints simply scale or trigger more NOPs. For DQN, a larger $k$ requires adapting the reward function (e.g., $P \propto k$) to balance the heavier cycle penalties. Due to training costs and our CPT model assumption ($P(L=1)=0$ for $\Delta t \ge 4$), we focused generalist training on $k=3$, leaving $k=50$ as an extreme stress-test.
\begin{figure}[h]
  \centering
